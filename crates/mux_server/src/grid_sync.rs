...
mod tests {
    ...
    fn fetch_scrollback_walks_older_and_newer_in_ascending_index_order() {
        ...
        let (newer, total) =
            fetch_scrollback_from_term(&term, 0, 1, 10).expect("fetch newer scrollback rows");
        assert_eq!(total, 3);
        assert_eq!(
        ...
        let (older, _) =
            fetch_scrollback_from_term(&term, 2, 0, 2).expect("fetch older scrollback rows");
        assert_eq!(
    ...
    fn fetch_scrollback_returns_total_but_no_rows_for_degenerate_requests() {
        ...
        let (rows, total) =
            fetch_scrollback_from_term(&term, 0, 1, 0).expect("fetch zero scrollback rows");
        assert!(rows.is_empty(), "count == 0 must yield no rows");
        assert_eq!(total, 3);

        let (rows, total) = fetch_scrollback_from_term(&term, 99, 1, 10)
            .expect("fetch out-of-range scrollback rows");
        assert!(
            rows.is_empty(),
        ...
        let (rows, total) =
            fetch_scrollback_from_term(&empty, 0, 1, 10).expect("fetch empty scrollback");
