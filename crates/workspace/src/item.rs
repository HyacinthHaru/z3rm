...
// spec §2.1：LEADER_UPDATE_THROTTLE 仅用于已删除的跟随者更新任务，现一并移除。
...
pub trait ItemHandle: 'static + Send {
    ...
    // spec §2.1：移除 followable 句柄转换，因 FollowableItem 协议依赖已删除的 proto/client。
...
impl<T: Item> ItemHandle for Entity<T> {
    ...
    fn added_to_pane(
        ...
        if old_item_pane.is_none() {
            // spec §2.1：协作/跟随功能依赖已删除的 client/proto，整体移除相关状态机。
            ...
            cx.observe_release_in(self, window, move |workspace, _, _, _| {
                ...
                // spec §2.1：send_follower_updates 已随跟随功能一并移除。
            })
            .detach();
        }
    ...
    // spec §2.1：to_followable_item_handle 随 FollowableItem 协议移除。
...
// spec §2.1 / §15.1：移除 FollowableItem / FollowableItemHandle / WeakFollowableItemHandle
// 及其关联类型（FollowEvent、Dedup、ViewId），因为全部依赖已删除的 client/proto
// 协作协议（CollaboratorId、PeerId、proto::View 等）。pane/tab 核心管理不依赖这些 trait。
