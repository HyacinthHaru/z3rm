...
impl Uri {
    ...
    /// `url::Url::from_file_path` is compiled only for unix, windows, redox and
    /// wasi, so on `wasm32-unknown-unknown` the URL is assembled here instead.
    ///
    /// The browser has no host filesystem; the paths that reach this are the
    /// ones the in-browser worktree made up, which are absolute and already
    /// slash-separated. Anything relative is rejected the same way upstream
    /// rejects it, rather than silently producing a URL that resolves
    /// elsewhere.
    ...
    pub fn from_file_path<P: AsRef<std::path::Path>>(path: P) -> Result<Self, ()> {
        ...
        let mut serialized = String::from("file://");
        for component in path.components() {
            ...
            for byte in segment.as_bytes() {
                // The unreserved set from RFC 3986, plus the sub-delims URL
                // leaves alone in a path segment. Everything else is escaped so
                // a space or a '#' cannot terminate the path early.
                if byte.is_ascii_alphanumeric()
                    || matches!(byte, b'-' | b'.' | b'_' | b'~' | b'$' | b'&' | b'+' | b',' | b'=' | b'@')
                {
                    serialized.push(*byte as char);
                } else {
                    serialized.push_str(&format!("%{byte:02X}"));
                }
            }
        }
        url::Url::parse(&serialized).map(Self).map_err(|_| ())
    }
}

/// `url::Url::to_file_path` is compiled only for unix, windows, redox and
/// wasi, so wasm gets the inverse of the constructor above.
#[cfg(target_family = "wasm")]
impl Uri {
    pub fn to_file_path(&self) -> Result<std::path::PathBuf, ()> {
        ...
        for segment in self.0.path_segments().ok_or(())? {
            if segment.is_empty() {
                continue;
            }
...
/// Reverses the escaping `from_file_path` applies. Rejects a truncated or
/// non-hex escape rather than dropping it, so a malformed URI cannot silently
